...
* 3_1 선언위치에 따른 변수의 종류[✏](#3_1)
* 3_2 클래스 변수와 인스턴스 변수[✏](#3_2)
* 3_3 메서드[✏](#3_3)
* 3_4 메서드의 선언과 구현[✏](#3_4)
* 3_5 메서드의 호출[✏](#3_5)
* 3_6 return문
* 3_7 JVM의 메모리 구조
* 3_8 기본형 매개변수와 참조형 매개변수
* 3_9 참조형 반환타입
* 3_10 재귀호출(recursive call)
* 3_11 클래스 메서드(static메서드)와 인스턴스 메서드
* 3_12 클래스 멤버와 인스턴스 멤버간의 참조와 호출

### 3_1 선언위치에 따른 변수의 종류[📑](#3_contents)<a id='3_1'></a>

* 변수는 `클래스변수`, `인스턴스변수`, `지역변수` 모두 세 종류가 있음.
  * 변수의 종류를 결정짓 는 중요한 요소는 '변수의 선언된 위치, 이므로 변수의 종류를 파악하기 위해서는 변수가 어느 영역에 선언되었는지를 확인하는 것이 중요함.
  * 멤버변수를 제외한 나머지 변수들 은 모두 지역 변수이며, 멤버변수 중 `static`이 붙은 것은 클래스변수, 붙지 않은 것은 `인스턴스 변수`임. 
* 아래의 그림에는 모두 3개의 int형 변수가 선언되어 있는데, `iv`와 `cv`는 클래스 영역에 선 언되어 있으므로 멤버변수임.
  * 그 중 cv는 키워드 static과 함께 선언되어 있으므로 클래스 변수이며, iv는 인스턴스변수임.
  * 그리고 lv는 메서드인 `method()` 내부, 즉 ‘메서드 영역’에 선언되어 있으므로 지역변수임.
...
* 클래스 영역에 선언되며, 클래스의 인스턴스를 생성할 때 만들어짐.
  * 그렇기 때문에 인스턴스 변수의 값을 읽어 오거나 저장하기 위해서는 먼저 인스턴스를 생성해야 함.
* 인스턴스는 독립적인 저장공간을 가지므로 서로 다른 값을 가질 수 있음.
  *  인스턴스마다 고유한 상태를 유지해야하는 속성의 경우, 인스턴스변수로 선언함.

#### 2. 클래스 변수(Class variable)

* 클래스 변수를 선언하는 방법은 인스턴스변수 앞에 `static`을 붙이기만 하면 됨.
  * 인스턴스마다 독립적인 저장공간을 갖는 인스턴스변수와는 달리, 클래스변수는 모든 인스턴스가 공통된 저장공간(변수)을 공유하게 됨.
  * 한 클래스의 모든 인스턴스들이 공통적인 값 을 유지해야하는 속성의 경우, 클래스변수로 선언해야 함.
* 클래스 변수는 인스턴스변수와 달리 인스턴스를 생성하지 않고도 언제라도 바로 사용할 수 있다는 특징이 있으며, `클래스이름.클래스변수`와 같은 형식으로 사용함.
  * 예를 들어 Variables클래스의 클래스변수 cv를 사용하려면 `Variables.cv`와 같이 하면 됨.
* 클래스가 메모리에 ‘로딩(loading)’될 때 생성되어 프로그램이 종료될 때 까지 유지되며, `public`을 앞에 붙이면 같은 프로그램 내에서 어디서나 접근할 수 있는 ‘전역변수(global variable)’의 성격을 갖게 됨. 
...
* 메서드 내에 선언되어 메서드 내에서만 사용 가능하며, 메서드가 종료되면 소멸되어 사용 할 수 없게 된다. 
  * `for문` 또는 `while문`의 블럭 내에 선언된 지역변수는, 지역변수가 선언된 블럭 {} 내에서만 사용 가능하며, 블럭 {}을 벗어나면 소멸되어 사용할 수 없게 됨.
  * 6장 이전에 선언한 변수들은 모두 지역변수임.

### 3_2 클래스 변수와 인스턴스 변수[📑](#3_contents)<a id='3_2'></a>

* 클래스변수와 인스턴스변수의 차이를 이해하기 위한 예로 카드 게임에 사용되는 카드를 클래스로 정의해봄.

![](./image/6_3_2-1.png)

> **그림6-4** 게임카드

카드 클래스를 작성하기 위해서는 먼저 카드를 분석해서 속성과 기능을 알아 내야함.

* 속성으로는 카드의 무늬, 숫자, 폭, 높이 정도를 생각할 수 있을 것임.
  * 이 중에서 어떤 속성을 클래스 변수로 선언할 것이며, 또 어떤 속성들을 인스턴스 변수로 선언할 것인지 한번 생각해봄.

![](./image/6_3_2-2.png)



### 3_3 메서드[📑](#3_contents)<a id='3_3'></a>

### 3_4 메서드의 선언과 구현[📑](#3_contents)<a id='3_4'></a>

### 3_5 메서드의 호출[📑](#3_contents)<a id='3_5'></a>

### 3_6 return문[📑](#3_contents)<a id='3_6'></a>

### 3_7 JVM의 메모리 구조[📑](#3_contents)<a id='3_7'></a>

### 3_8 기본형 매개변수와 참조형 매개변수[📑](#3_contents)<a id='3_8'></a>

### 3_9 참조형 반환타입[📑](#3_contents)<a id='3_9'></a>

### 3_10 재귀호출(recursive call)[📑](#3_contents)<a id='3_10'></a>

### 3_11 클래스 메서드(static메서드)와 인스턴스 메서드[📑](#3_contents)<a id='3_11'></a>

### 3_12 클래스 멤버와 인스턴스 멤버간의 참조와 호출[📑](#3_contents)<a id='3_12'></a>



## 5_생성자(constructor)[📑](#contents)<a id='5'></a>

### 📑contents<a id='5_contents'></a>
...
### 5_1 생성자란?[📑](#5_contents)<a id='5_1'></a>
...
### 5_2 기본 생성자(default constructor)[📑](#5_contents)<a id='5_2'></a>
...
### 5_3 매개변수가 있는 생성자[📑](#5_contents)<a id='5_3'></a>
...
### 5_4 생성자에서 다른 생성자 호출하기 - this(), this[📑](#5_contents)<a id='5_4'></a>
...
### 5_5 생성자를 이용한 인스턴스의 복사[📑](#5_contents)<a id='5_5'></a>
